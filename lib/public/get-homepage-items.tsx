...
import { CollectionType } from "@/components/items/shared/item-types";
...
export async function getHomepageItems(
  ...
  console.log("Total items returned from Supabase:", data.length);

  data.forEach((item, index) => {
    console.log(
      `Item [${index}] Name: "${item.name}" | Collection in DB: "${item.collection}" | Targeted: "${collection}"`,
    );
  });
  // --- DIAGNOSTIC LOG END ---
  const featured = data
